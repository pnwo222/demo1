  ...
  global.SnowyPrototypeComponents.SnowyAnnotationToolbar = {
    ...
    template: `          <div class="annotation-toolbar" :class="{ collapsed: annotationToolbarCollapsed }">
            ...
            <template v-if="!annotationToolbarCollapsed">
              ...
              <button class="annotation-tool-button annotation-tool-action" :class="{ primary: annotationEnabled, expanded: annotationEnabled }" type="button" :title="annotationEnabled ? '关闭标注' : '开启标注'" @click="toggleAnnotationMode">
                ...
                <span class="annotation-tool-label">{{ annotationEnabled ? '正在标注' : '开启标注' }}</span>
              </button>
              <button class="annotation-tool-button annotation-tool-action" type="button" title="查看和编辑当前页面整体需求" @click="openRequirementDrawer">
                <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
                ...
                <span class="annotation-tool-label">页面需求</span>
              </button>
              <button class="annotation-tool-button annotation-tool-action" type="button" title="另存为包含当前标注的 HTML" @click="saveAsAnnotatedHtml">
                <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
                ...
                <span class="annotation-tool-label">另存为</span>
              </button>
              <span class="annotation-tool-separator annotation-tool-danger-separator" aria-hidden="true"></span>
              <button class="annotation-tool-button annotation-tool-action danger" type="button" title="删除当前页面全部用户标注和修改" @click="deleteAllAnnotations">
                <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
                  <path d="M4 7h16M9 7V4h6v3M7 7l1 13h8l1-13M10 11v5M14 11v5" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" />
                </svg>
                <span class="annotation-tool-label">全部删除</span>
              </button>
              </div>
            </template>
          </div>`
