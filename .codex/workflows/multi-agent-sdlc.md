...
- 本项目已将 `Agents365-ai/365-skills` 安装到 `.codex/skills/`。任何阶段需要绘制流程图、架构图、模块图、ERD、状态机图、时序图、任务图、DAG、依赖图或其他系统可视化时，必须优先使用 `.codex/skills/mermaid-skill`，并在使用前读取该 skill 的 `SKILL.md`。
- 需要可编辑 Draw.io 文件、复杂样式、泳道、厂商图标、精细布局或导出 PNG/SVG/PDF/JPG 时，改用 `.codex/skills/drawio-skill`；需要 PlantUML/UML/C4 语义时，改用 `.codex/skills/plantuml-skill`；需要手绘白板风格时，改用 `.codex/skills/excalidraw-skill`。
- 图形产物必须按所选 skill 的校验流程执行。Mermaid/PlantUML 通过 `mmdc`、Kroki 或对应 skill 指定方式校验后再导出；Draw.io/Excalidraw 必须按 skill 要求检查渲染，避免重叠、裁切、空白或不可读。
- 技术设计、数据设计、任务拆分、并行开发和项目工作流程图中出现的图表，必须在产物中记录使用的 skill、源文件路径和导出文件路径。

触发规则：

- 用户输入“完成/开发/实现/新增/修复/优化 + 某功能”时，必须视为项目工作流入口，先进入前置 1 开发环境检测，不得直接编码。
...
| 设计项 | 必须明确的问题 | 常见产物 |
| --- | --- | --- |
| 模块边界 | 各业务模块分别由哪些前后端或服务负责 | 使用 365 diagram skills 生成的架构图、模块图 |
| 接口契约 | 前端如何调用后端 API，请求、响应、错误码、分页、权限 | OpenAPI、API 文档 |
| 数据模型 | 核心实体、关系、生命周期、历史快照 | 使用 365 diagram skills 生成的 ERD、Migration |
| 状态机 | 哪些状态可以流转，谁触发，影响什么资源 | 使用 365 diagram skills 生成的状态机图 |
