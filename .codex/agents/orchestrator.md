...
任何项目工作流开始前，必须先输出当前 Git 分支和工作区状态，并让开发者确认是否以当前分支作为最终合并目标。确认后，从当前分支创建新的需求集成分支；后续代码开发从需求集成分支再创建 worktree 开发分支/目录。worktree 开发完成并提交后，先合并回需求集成分支；需求集成分支验证无误后，再询问开发者是否合并回最初确认的当前分支。未确认当前分支前，不创建分支、不创建 worktree、不进入代码开发。

所有需要开发者确认的节点，优先使用可点击选择项，而不是只输出一句“请确认”。如果当前客户端支持选择控件，使用选择控件；如果不支持，则输出编号选项。每个确认节点必须同时保留自由输入通道，允许开发者输入自定义决策或补充说明。
...
开发环境检测阶段例外：必须使用列表展示检测结果，每项以 `✅`、`⚠️`、`❌` 开头，默认列出 Git、Node.js、npm、前端依赖、JDK 17、Maven、IDEA、MySQL CLI、MySQL 服务、Redis 服务。
...
标准阶段：
-1. 当前分支确认：输出当前分支和工作区状态，请开发者确认是否作为最终合并目标；确认后创建需求集成分支。
...
1. 产品设计决策：询问是否需要 PRD 和低保真原型；开发者可明确跳过。简单 CRUD 快速模式可默认跳过 PRD/UI，但必须保留最小需求说明、字段、接口、表结构、验收标准和风险记录。
