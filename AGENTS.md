...
- 低保真原型必须参考现有框架的信息架构、布局、导航、表格、表单、弹窗、抽屉、状态和权限交互，不得做成过度简陋的纯线框。后管原型和 H5/移动端原型必须分开输出；后管必须以完整原始 Demo 金标为基础，并复用 `prototype-demo-framework/components/` 中抽取的原始组件，不得用精简 Schema 渲染器重画。H5 如框架未补充则标记 `H5 框架待补充`。原型必须严格覆盖全部模块、页面、菜单、字段、操作、状态、异常和权限场景，并输出覆盖矩阵。
