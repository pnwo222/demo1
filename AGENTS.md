...
- 后管原型生成 HTML 前必须先输出“需求到原型页面蓝图”：逐个独立页面列出需求编号、菜单路径、路由路径、权限标识、页面类型、参考 Snowy 页面、查询字段、表格字段、详情字段、新增字段、编辑字段、操作按钮、状态/异常、权限差异、字段展示形态和点击交互。没有蓝图、蓝图字段不来自需求、或多个业务页面共用一套万能查询/表格/表单/抽屉时，原型视为不合格，必须退回 Product 阶段重做。
...
- 如涉及后管、后台、管理端或运营端页面且未跳过原型，已输出需求到原型页面蓝图；每个独立页面的查询字段、表格字段、详情字段、新增字段、编辑字段、操作按钮、状态/异常、权限和字段展示形态均来自需求或明确标记待确认；覆盖矩阵能追溯到蓝图和 HTML。
