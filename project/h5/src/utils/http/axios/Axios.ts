...
export class VAxios {
  ...
  private setupInterceptors() {
    ...
    this.axiosInstance.interceptors.request.use((config: InternalAxiosRequestConfig) => {
      ...
      // @ts-expect-error
      const ignoreCancelTokenCustom = config?.requestOptions?.ignoreCancelToken ?? true

      if (!ignoreCancelTokenCustom) {
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-expect-error
        axiosCanceler.addPending(config, config?.requestOptions)
      }
  ...
  // support form-data
  supportFormData(config: AxiosRequestConfig = {}, options?: RequestOptions) {
  ...
  getURL(config: AxiosRequestConfig, options?: RequestOptions) {
    ...
    conf = this.supportFormData(conf, options)
    return `${conf.url}?${qs.stringify(conf.data)}`
  }
