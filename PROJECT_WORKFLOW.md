...
flowchart TD
    ...
    L2A --> L2B{"蓝图校验通过"}
    L2B -- 否 --> L2
    L2B -- 是 --> L2C["复用未变化的已确认产物<br/>超过 10 页按模块生成独立配置<br/>共享 HTML 由单一 Owner 汇总"]
    L2C --> L3["按蓝图生成显式业务 Schema<br/>逐页声明查询、表格、详情、表单和操作组件<br/>输出框架参考、菜单映射和覆盖矩阵"]
    L3 --> L4["组件注册表校验后运行 build-prototype.mjs<br/>统一嵌入 Snowy 组件与标注运行时<br/>禁止复制手改 HTML 或绕过组件包"]
...
- 涉及后管、后台、管理端或运营端且未跳过原型时，必须使用 `.codex/skills/snowy-admin-prototype-designer` 并套用 `.codex/workflows/admin-prototype-design-workflow.md`，先输出“需求到原型页面蓝图”。蓝图必须逐个独立页面列出原始需求摘录、原子需求清单、同步字段、展示字段、筛选字段、查询字段、表格字段、详情字段、新增字段、编辑字段、状态字段、敏感字段、操作按钮、状态/异常、权限差异、字段展示形态和点击交互；每个字段、按钮、状态和权限必须标记来源。蓝图通过后生成显式业务 Schema，每个字段和操作必须选择注册组件，再通过 `build-prototype.mjs` 输出单 HTML。五段组件包源码和哈希必须与 `src/` 一致，业务差异只允许进入 Schema；缺失组件必须扩展、注册并测试。禁止复制手改 Demo、平行标注实现、万能字段集或覆盖基础 CSS。必须通过静态校验、运行时交互校验和覆盖矩阵验收。
...
- 后管原型必须由组件包构建器生成。业务 Schema 应按字段语义选用 `QueryForm`、`DataTable`、`FormDrawer`、`DetailDrawer`、上传、状态、开关、附件、金额、脱敏文本和动作组件；每页配置独立完整。禁止手改生成 HTML、原生文件输入、字段名猜测和万能业务字段集。
